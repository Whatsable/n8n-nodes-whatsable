import { WhatsAbleTriggerApi } from '../../credentials/WhatsAbleTriggerApi.credentials';
import { IAuthenticateGeneric, ICredentialTestRequest } from 'n8n-workflow';

describe('WhatsAbleTriggerApi Credentials', () => {
	let credentials: WhatsAbleTriggerApi;

	beforeAll(() => {
		credentials = new WhatsAbleTriggerApi();
	});

	test('should have the correct name', () => {
		expect(credentials.name).toBe('whatsAbleTriggerApi');
	});

	test('should have the correct display name', () => {
		expect(credentials.displayName).toBe('WhatsAble Trigger API');
	});
	...
	test('should have required properties', () => {
		...
		expect(apiKeyProperty.description).toBe('API Key for WhatsAble Trigger');
