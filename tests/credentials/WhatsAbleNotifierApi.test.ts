import { WhatsAbleNotifierApi } from '../../credentials/WhatsAbleNotifierApi.credentials';
import { IAuthenticateGeneric, ICredentialTestRequest } from 'n8n-workflow';

describe('WhatsAbleNotifierApi Credentials', () => {
	let credentials: WhatsAbleNotifierApi;

	beforeAll(() => {
		credentials = new WhatsAbleNotifierApi();
	});

	test('should have the correct name', () => {
		expect(credentials.name).toBe('whatsAbleNotifierApi');
	});

	test('should have the correct display name', () => {
		expect(credentials.displayName).toBe('WhatsAble Notifier Trigger API');
	});
	...
	test('should have required properties', () => {
		...
		expect(apiKeyProperty.description).toBe('API Key for WhatsAble Notifier');
	...
	test('should have correct test configuration', () => {
		...
		expect(test.request.baseURL).toContain('/api:gncnl2D6');
	});
}); 